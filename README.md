...
This is the way to create a new vertex in a _namespace_. A _namespace_ is a collection of vertexes on which _security rules and permissions_ can be applied and all the vertexes belonging to this namespace will follow the rules.
...
### Appbase.ref()
Create an `Appbase Reference` pointing to the vertex at a path. 
...
A _Path_ in Appbase represents a chain of elements, separated with '/', and it finally points a vertex. The _base-url_ is a unique string for the Application, and the first element after the url represents a _namespace_, and following elements are vertexes. 
